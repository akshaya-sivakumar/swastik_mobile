...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return FadeTransition(
      ...
      child: Container(
        margin: EdgeInsets.only(left: 10.w, top: 8.w, right: 8.w),
        child: CustomPaint(
...
class DropdownButtonState<T> extends State<CustomDropdownButton<T>>
  ...
  void initState() {
    ...
    _updateSelectedIndex(widget.value);
  ...
  void _updateSelectedIndex(values) {
    ...
    for (int itemIndex = 0; itemIndex < widget.items.length; itemIndex++) {
      if (widget.items[itemIndex].value == (values)) {
  ...
  void _handleTap() {
    ...
    Navigator.push(context, _dropdownRoute!)
      ...
      if (widget.onChanged != null) {
        ...
        // setState(() {});
        _updateSelectedIndex(newValue.result);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget result = DefaultTextStyle(
      style: _textStyle,
      child: Container(
        padding: padding.resolve(Directionality.of(context)).copyWith(left: 10),
